...
```bash
# Interactive setup (command bar, GitHub Actions extension)
tmux-web setup
...
Secrets in `~/.tmux-web/.env` are loaded automatically when you run `tmux-web`. The GitHub Actions sidebar uses the [GitHub CLI](https://cli.github.com/) (`gh auth login`); for headless or server use you can set `GH_TOKEN` or `GITHUB_PAT` there instead.
...
- **GitHub CLI** (`gh`) on your PATH if you use the GitHub Actions sidebar extension
