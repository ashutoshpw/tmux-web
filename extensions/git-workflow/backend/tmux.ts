...
const PANE_FORMAT =
  '#{pane_id}\t#{pane_index}\t#{window_index}\t#{pane_current_path}\t#{pane_current_command}\t#{alternate_on}';
...
export function getActivePaneInfo(session: string): ActivePaneInfo | null {
  try {
    ...
    const [paneId, paneIndex, windowIndex, panePath, paneCommand, alternateOn] = out.split('\t');
